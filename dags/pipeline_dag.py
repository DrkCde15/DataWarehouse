"""DAG Airflow para orquestração do pipeline API → Data Warehouse."""
...
def extract_data(**context) -> None:
    """Busca dados da API e salva em staging via XCom (JSON serializado)."""
    ...
    endpoint = params.get("endpoint", "/coins/markets")
    pagination_type = params.get("pagination_type", "offset")
    page_size = params.get("page_size", API_PAGE_SIZE)

    # query_params explícito tem prioridade; senão usa vs_currency
    ...
    records = client.get_all_pages(
        ...
        page_size=page_size,
        params=query_params,
        results_key=params.get("results_key", "results"),
    )

    # Extrair sub-registros via record_path (ex: "coins.item" para trending)
    record_path = params.get("record_path")
    if record_path and records:
        for key in record_path.split("."):
            if isinstance(records, list) and len(records) > 0:
                records = [r.get(key, r) if isinstance(r, dict) else r for r in records]
                if isinstance(records[0], list):
                    records = [item for sublist in records for item in sublist]
            elif isinstance(records, dict):
                records = records.get(key, records)

    # Achatar dicts aninhados se solicitado
    if params.get("flatten_nested") and records:
        from src.transformers.cleaner import DataCleaner
        records = DataCleaner.flatten_nested_dict(records)
...
def transform_data(**context) -> None:
    ...
    # Serializar datas, dicts e converter unix timestamps
    for col in df_clean.columns:
        if pd.api.types.is_datetime64_any_dtype(df_clean[col]):
            df_clean[col] = df_clean[col].astype(str)

    # Converter unix timestamps (integers > 1e9) para ISO string
    for col in df_clean.columns:
    ...
    # Converter dicts/listas para JSON string (para colunas JSONB)
    for col in df_clean.columns:
...
def load_data(**context) -> None:
    """Insere dados limpos no PostgreSQL."""
    ...
    target_table = context["params"].get("target_table", "staging.coingecko_coins")
    ...
    try:
        ...
        # Manter colunas que existem em ambos
        cols_to_keep = [c for c in df.columns if c in valid_cols]
        if set(df_cols) != valid_cols:
            dropped = df_cols - valid_cols
            logger.warning(f"Colunas ignoradas (não existem na tabela): {dropped}")
        if cols_to_keep:
            df = df[cols_to_keep]
...
def register_job_start(**context) -> None:
    """Registra início do job no metadata."""
    from src.loaders.postgres_loader import PostgresLoader
...
def register_job_completion(**context) -> None:
    """Registra conclusão do job no metadata."""
    from src.loaders.postgres_loader import PostgresLoader
...
def register_job_failure(**context) -> None:
    """Registra falha do job no metadata."""
    from src.loaders.postgres_loader import PostgresLoader
...
with DAG(
    dag_id="pipeline_etl_api",
    default_args=DEFAULT_ARGS,
    description="Pipeline ETL completo: API → Staging → DW",
    schedule_interval="@daily",
    start_date=datetime(2024, 1, 1),
    catchup=False,
    max_active_runs=1,
    tags=["etl", "api", "data_warehouse"],
    params={
