"""DAG para extrair moedas em alta do CoinGecko (/search/trending)."""
...
def extract_data(**context) -> None:
    ...
    endpoint = params.get("endpoint", "/search/trending")
    ...
    records = client.get_all_pages(
        ...
        page_size=page_size,
        params=query_params,
        results_key=params.get("results_key", "coins"),
    )
...
def load_data(**context) -> None:
    ...
    target_table = context["params"].get("target_table", "staging.coingecko_trending")
...
with DAG(
    dag_id="pipeline_coingecko_trending",
    default_args=DEFAULT_ARGS,
    description="Pipeline ETL: CoinGecko /search/trending -> staging.coingecko_trending",
    schedule_interval="@daily",
    start_date=datetime(2024, 1, 1),
    catchup=False,
    max_active_runs=1,
    tags=["etl", "api", "coingecko", "trending"],
    params={
        "endpoint": "/search/trending",
        "pagination_type": "none",
        "page_size": 250,
        "target_table": "staging.coingecko_trending",
